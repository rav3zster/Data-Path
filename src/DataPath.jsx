...
import {
  ...
  ArrowRight, BookOpen, Settings, Calendar,
} from "lucide-react";

// ─── storage (no Capacitor import at top level — avoids browser crash) ────────
...
// ─── themes ───────────────────────────────────────────────────────────────────
const THEMES = {
  midnight: { label:"Midnight", dark:true, swatch:["#070b14","#22d3a5"], bg:"#070b14", surf:"#0d1321", surf2:"#111827", surf3:"#1a2540", navBg:"rgba(13,19,33,0.92)", onAccent:"#070b14", bdr:"rgba(148,163,184,0.07)", bdr2:"rgba(148,163,184,0.13)", txt:"#f1f5f9", txt2:"#94a3b8", txt3:"#475569", teal:"#22d3a5", purple:"#8b7cf8", coral:"#f87171", amber:"#fb923c" },
  carbon:   { label:"Carbon",   dark:true, swatch:["#0a0a0b","#22d3a5"], bg:"#0a0a0b", surf:"#151517", surf2:"#1d1d20", surf3:"#29292e", navBg:"rgba(21,21,23,0.92)",  onAccent:"#0a0a0b", bdr:"rgba(255,255,255,0.06)", bdr2:"rgba(255,255,255,0.12)", txt:"#fafafa", txt2:"#a1a1aa", txt3:"#5b5b63", teal:"#22d3a5", purple:"#8b7cf8", coral:"#f87171", amber:"#fb923c" },
  ocean:    { label:"Ocean",    dark:true, swatch:["#04141f","#38bdf8"], bg:"#04141f", surf:"#0a2433", surf2:"#0e2e40", surf3:"#16435c", navBg:"rgba(10,36,51,0.92)",  onAccent:"#04141f", bdr:"rgba(125,211,252,0.08)", bdr2:"rgba(125,211,252,0.16)", txt:"#ecfeff", txt2:"#94b8c9", txt3:"#4b6b7d", teal:"#22d3a5", purple:"#8b7cf8", coral:"#f87171", amber:"#fb923c" },
  plum:     { label:"Plum",     dark:true, swatch:["#120a1f","#c084fc"], bg:"#120a1f", surf:"#1c1030", surf2:"#251640", surf3:"#34215a", navBg:"rgba(28,16,48,0.92)",  onAccent:"#120a1f", bdr:"rgba(192,132,252,0.08)", bdr2:"rgba(192,132,252,0.16)", txt:"#faf5ff", txt2:"#b8a9cf", txt3:"#6b5b85", teal:"#22d3a5", purple:"#a78bfa", coral:"#f87171", amber:"#fb923c" },
  forest:   { label:"Forest",   dark:true, swatch:["#08140f","#34d399"], bg:"#08140f", surf:"#0e1f17", surf2:"#142a20", surf3:"#1d3d2e", navBg:"rgba(14,31,23,0.92)",  onAccent:"#08140f", bdr:"rgba(134,239,172,0.07)", bdr2:"rgba(134,239,172,0.14)", txt:"#f0fdf4", txt2:"#9bb5a6", txt3:"#4d6b58", teal:"#22d3a5", purple:"#8b7cf8", coral:"#f87171", amber:"#fb923c" },
  daylight: { label:"Daylight", dark:false,swatch:["#ffffff","#0d9488"], bg:"#eef2f7", surf:"#ffffff", surf2:"#f1f5f9", surf3:"#e2e8f0", navBg:"rgba(255,255,255,0.92)",onAccent:"#ffffff", bdr:"rgba(15,23,42,0.08)",   bdr2:"rgba(15,23,42,0.16)",   txt:"#0f172a", txt2:"#475569", txt3:"#94a3b8", teal:"#0d9488", purple:"#7c5cf0", coral:"#e11d48", amber:"#d97706" },
};
const THEME_LIST = Object.entries(THEMES).map(([id,t]) => ({ id, label:t.label, swatch:t.swatch }));
const DEFAULT_THEME = "midnight";
const ThemeCtx = createContext(THEMES[DEFAULT_THEME]);
...
const TASKS = WRAW.flatMap((w,wi) => w.ts.map((t,ti) => ({ id:wi*7+ti+1, day:wi*7+ti+1, week:wi+1, phase:w.ph, color:w.cl, weekTopic:w.tp, ...t })));
...
const DEF_STATE = { completed:{}, startDate:TODAY, customBlocks:{}, theme:DEFAULT_THEME };

// ─── shared components (read theme via context) ───────────────────────────────
...
const Tag = ({ children, color, bg }) => {
  const C = useC();
  return <span style={{ display:"inline-flex", alignItems:"center", gap:3, padding:"2px 8px", borderRadius:99, fontSize:10, fontWeight:500, color:color||C.txt2, background:bg||C.surf3, border:`1px solid ${color?color+"30":C.bdr}` }}>{children}</span>;
};
...
export default function DataPath() {
  ...
  const C = useMemo(() => THEMES[st.theme] || THEMES[DEFAULT_THEME], [st.theme]);
  ...
  const save = useCallback((ns) => { try { store.set(SK, JSON.stringify(ns)); } catch { /* ignore */ } }, []);
  ...
  const streak = useMemo(() => {
    const isDone = dn => { const dt=TASKS.filter(t=>t.day===dn); return dt.length>0&&dt.every(t=>completed[t.id]); };
    let s=0, start=isDone(dayNum)?dayNum:dayNum-1;
    for (let dn=start; dn>=1; dn--) { if(isDone(dn)) s++; else break; }
    return s;
  }, [completed, dayNum]);
  ...
  const filteredTasks = useMemo(() => TASKS.filter(t=>(filterPh===0||t.phase===filterPh)&&(filterWk===0||t.week===filterWk)), [filterPh,filterWk]);
  const byWeek = useMemo(() => {
    const g={};
    filteredTasks.forEach(t => { if(!g[t.week]) g[t.week]={topic:t.weekTopic,color:t.color,tasks:[]}; g[t.week].tasks.push(t); });
    return g;
  }, [filteredTasks]);
  ...
  if (!loaded) return (
    <ThemeCtx.Provider value={C}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:C.bg, color:C.teal, fontFamily:"monospace", fontSize:14 }}>
        <div style={{ textAlign:"center" }}><div style={{ fontSize:32, marginBottom:12 }}>⚡</div><div>Loading DataPath...</div></div>
      </div>
    </ThemeCtx.Provider>
  );
  ...
  const renderSettings = () => (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:16 }}>
      <div style={{ background:C.surf, border:`1px solid ${C.bdr2}`, borderRadius:16, padding:24, width:"100%", maxWidth:340, maxHeight:"90vh", overflowY:"auto" }}>
  ...
  const renderRoadmap = () => (
    <div style={{ padding:"20px 16px 24px", maxWidth:680, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", marginBottom:24, background:C.surf, borderRadius:12, overflow:"hidden", border:`1px solid ${C.bdr}` }}>
        {phStats.map((p,i) => (
      ...
      {PHASES.map(ph => {
        ...
        return (
          <div key={ph.id} style={{ marginBottom:10 }}>
            <div onClick={() => setOpenPhase(isOpen?null:ph.id)} style={{ background:C.surf, border:`1px solid ${isOpen?ph.color+"50":C.bdr}`, borderLeft:`3px solid ${ph.color}`, borderRadius:12, padding:16, cursor:"pointer", display:"flex", alignItems:"center", gap:12, userSelect:"none" }}>
                  ...
                  return (
                    <div key={wk.wkNum} style={{ background:C.surf2, border:`1px solid ${C.bdr}`, borderLeft:`2px solid ${ph.color}40`, borderRadius:10, padding:14, marginBottom:8 }}>
                      ...
                      <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:10 }}>
                        {wkTasks.map(t => (
                          <div key={t.id} onClick={() => toggle(t.id)} style={{ display:"flex", alignItems:"center", gap:4, padding:"4px 8px", borderRadius:6, background:completed[t.id]?ph.color+"20":C.surf3, border:`1px solid ${completed[t.id]?ph.color+"40":C.bdr}`, cursor:"pointer" }}>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:14, marginBottom:16 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:12 }}>
            {[{ id:0, label:"All", color:C.txt2 }, ...PHASES.map(p => ({ id:p.id, label:p.title, color:p.color }))].map(f => (
              <div key={f.id} onClick={() => { setFilterPh(f.id); setFilterWk(0); }} style={{ padding:"5px 12px", borderRadius:99, fontSize:11, fontWeight:500, cursor:"pointer", background:filterPh===f.id?f.color+"20":C.surf2, border:`1px solid ${filterPh===f.id?f.color:C.bdr}`, color:filterPh===f.id?f.color:C.txt2 }}>{f.label}</div>
            ))}
          </div>
          {filterPh>0 && (
            <>
                ...
                {group.tasks.map(task => (
                  <div key={task.id} style={{ borderBottom:`1px solid ${C.bdr}` }}>
                    <div style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"12px 16px", cursor:"pointer" }} onClick={() => setExpandTask(expandTask===task.id?null:task.id)}>
                      <div onClick={e => { e.stopPropagation(); toggle(task.id); }}><Cb done={!!completed[task.id]} onToggle={() => toggle(task.id)} color={task.color} /></div>
  ...
  const renderSchedule = () => {
    const weekDates = (() => {
      const dt = new Date(selDate+"T00:00:00"), day = dt.getDay();
    ...
    return (
      <div style={{ padding:"20px 16px 24px", maxWidth:680, margin:"0 auto" }}>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:12, marginBottom:16 }}>
          <div style={{ display:"flex", gap:4 }}>
            {weekDates.map(d => {
              ...
              const dNum=dayDiff(startDate,d)+1, inPlan=dNum>=1&&dNum<=84;
              ...
              return (
                <div key={d} onClick={() => setSelDate(d)} style={{ flex:1, textAlign:"center", padding:"8px 2px", borderRadius:10, cursor:"pointer", background:isSel?C.teal+"20":isToday?C.surf2:"transparent", border:`1px solid ${isSel?C.teal:isToday?C.bdr2:"transparent"}` }}>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:16, marginBottom:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:selDayNum?C.teal:C.txt3, marginTop:3 }}>{selDayNum?`Day ${selDayNum} of 84`:"Outside your 84-day plan"}</div>
            </div>
        ...
        {selDayTasks.length>0 && (
          <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:16, marginBottom:14 }}>
            <div style={{ fontSize:12, fontWeight:600, color:C.txt, marginBottom:12, display:"flex", alignItems:"center", gap:6 }}><Target size={14} color={C.teal} /> Assigned Tasks for this Day</div>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:16 }}>
          <div style={{ fontSize:12, fontWeight:600, color:C.txt, marginBottom:12, display:"flex", alignItems:"center", gap:6 }}><Calendar size={14} color={C.purple} /> My Study Plan for this Day</div>
  ...
  const renderProgress = () => {
    ...
    const recentDays=Array.from({ length:28 }, (_,i) => {
      const d=addDays(TODAY,-(27-i)), dn=dayDiff(startDate,d)+1;
      const dt=(dn>=1&&dn<=84)?TASKS.filter(t=>t.day===dn):[];
      return { date:d, tasks:dt.length, done:dt.filter(t=>completed[t.id]).length };
    });
    return (
      <div style={{ padding:"20px 16px 24px", maxWidth:680, margin:"0 auto" }}>
        <div style={{ marginBottom:24 }}>
        ...
        <div style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderRadius:12, padding:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(28,1fr)", gap:3 }}>
            {recentDays.map((d,i) => {
              ...
              return <div key={i} title={`${fmtShort(d.date)}: ${d.done}/${d.tasks}`} style={{ aspectRatio:"1", borderRadius:3, background:bg }} />;
            })}
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:8 }}>
      ...
      {RESOURCES.map(cat => (
        <div key={cat.cat} style={{ background:C.surf, border:`1px solid ${C.bdr}`, borderLeft:`3px solid ${cat.color}`, borderRadius:12, padding:16, marginBottom:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {cat.items.map(item => (
              <a key={item.url} href={`https://${item.url}`} target="_blank" rel="noreferrer" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, padding:"10px 12px", background:C.surf2, borderRadius:8, border:`1px solid ${C.bdr}`, textDecoration:"none" }}>
                ...
                <ExternalLink size={11} color={cat.color} style={{ flexShrink:0 }} />
              </a>
            ))}
          </div>
        </div>
  ...
  // ── render ────────────────────────────────────────────────────────────────────
  const pages = { dashboard:renderDashboard, roadmap:renderRoadmap, tasks:renderTasks, schedule:renderSchedule, progress:renderProgress, resources:renderResources };
